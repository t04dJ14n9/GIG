...
func Lessons() []Lesson {
	lessons := make([]Lesson, len(lessonCatalog))
	copy(lessons, lessonCatalog)
	for i := range lessons {
		lessons[i].Quiz.Options = append([]string(nil), lessonCatalog[i].Quiz.Options...)
	}
	return lessons
}
